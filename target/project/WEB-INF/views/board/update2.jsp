...
<head>
    ...
    <script>
        ...
        // }
        const imageUpdate = (i) =>{
            const imageName = document.getElementById("imageName"+i).value;
            let data = new FormData();
            data.append("imageName",imageName);
            data.append("i_id", i);
            $.ajax({
                type:"post",
                url:"/board/image-name",
                dataType: 'json',
                data: data,
                contentType: false,
                processData: false,
        ...
        const imageChange = (i) => {
            ...
            $.ajax({
                ...
                cache: false,
                success: function (result){
                    document.getElementById("imageName"+result.i_id).value = result.imageName;
                    $("#img"+result.i_id).attr("src", "${pageContext.request.contextPath}/upload/" + result.imageManageName);
                    console.log(result);
                },error: function (){
                    alert("에러");
                }
                    ...
                    <td>
                        ...
                        <table class="table table-light border-dark border border-1">
                            ...
                            <c:forEach var="image" items="${imageList}">
                                <tr>
                                    <td class="col-sm-1">
                                        <img src="${pageContext.request.contextPath}/upload/${image.imageManageName}" id="img${image.i_id}" alt="${image.imageName}" height="100" width="100">
                                    </td>
                                    <td class="col-sm-1">
